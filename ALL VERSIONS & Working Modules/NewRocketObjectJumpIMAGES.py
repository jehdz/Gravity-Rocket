...
def main():
    ...
    playerY = windowY/2
    obstacles = [
        Image(Point(0,0), "star.gif"),
        Image(Point(0,-200), "star.gif"),
        Image(Point(0,-400), "star.gif"),
        Image(Point(0,-600), "star.gif"),
        Image(Point(0,-800), "star.gif"),
        Image(Point(0,-1000), "star.gif")
    ]
    obstacleLocations = [
        [0,0],
        [0,-200],
    ...
    while True:
        ...
        for i in range(len(obstacles)-1,-1,-1):
            obstacleLocations[i][0], obstacleLocations[i][1] = moveToImage(obstacles[i], obstacleLocations[i][0], obstacleLocations[i][1], obstacleSpeeds[i], 0, obstacleLocations[i][0]-cameraX+obstacleSpeeds[i], obstacleLocations[i][1]-cameraY)
...
def moveToImage(shape, shapeX, shapeY, dx, dy, x, y):
    shape.move(x-shape.getAnchor().getX(), y-shape.getAnchor().getY())
    return shapeX + dx, shapeY + dy
